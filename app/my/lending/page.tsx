...
import {
  Package, Calendar, MapPin, CheckCircle2, Clock, Star,
...
interface Order {
  ...
  total_rent: number; deposit: number; total_amount: number;
  platform_fee: number; seller_amount: number; late_fee: number;
  status: string; buyer_name: string; buyer_id: number; seller_reviewed: number;
  buyer_confirmed: number; seller_confirmed: number;
  handover_buyer_photos: string; handover_seller_photos: string;
  return_buyer_photos: string; compensation_requested: number;
}
...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  returning: { label: '等待確認歸還', color: 'bg-orange-100 text-orange-700' },
...
export default function MyLendingPage() {
  ...
  const fetchOrders = useCallback(async () => {
    if (!token) return;
    const res = await fetch('/api/orders?role=seller', { headers: { Authorization: `Bearer ${token}` } });
  ...
  async function handleComplete(orderId: number) {
    if (!confirm('確認商品已歸還完好？這將完成本次租借。')) return;
  ...
  async function handleDispute(orderId: number) {
    ...
    try {
      ...
      if (res.ok) { setDisputeOrderId(null); setDisputeReason(''); fetchOrders(); }
    } finally {
      setSubmittingAction(false);
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-primary">出租管理</h1>
        <Link href="/my/listings" className="text-sm text-wood hover:underline">管理我的商品</Link>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="bg-card border border-border rounded-2xl p-16 text-center text-subtle">
          ...
          <p className="text-lg font-medium text-muted">尚無出租紀錄</p>
          <p className="text-sm mt-1 mb-6">上架商品後，別人的租借訂單會顯示在這裡</p>
        ...
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            const returnPhotos: string[] = JSON.parse(order.return_buyer_photos || '[]');
              ...
              <div key={order.id} className="bg-card border border-border rounded-2xl p-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-subtle mt-1">租借人：{order.buyer_name}</p>
                    ...
                    {['active', 'returning', 'completed'].includes(order.status) && (
                      <p className="text-xs text-muted mt-1">
                        平台收取5%手續費，您實得 <span className="font-medium text-primary">NT${(order.seller_amount ?? Math.round(order.total_rent * 0.95)).toLocaleString()}</span>
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  <div className={`mt-4 flex items-start gap-2 text-sm px-4 py-3 rounded-xl ${
                    ...
                    <div>
                      <p className="font-medium">
                        租借人已超時 {overdueDays} 天 — 可扣押金 {lateFeeRate}%（NT${Math.round(order.deposit * lateFeeRate / 100).toLocaleString()}）
                      </p>
                      {overdueDays >= 7 && (
                        <p className="text-xs mt-0.5">超時7天以上，如租借人持續未歸還，平台可協助賣家提出報案。</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                {order.status === 'disputed' && (
                  <div className="mt-4 flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-xl">
                    ...
                    <p>押金糾紛申請已送出，平台客服正在處理中。押金暫時凍結，請等候通知。</p>
                  </div>
                )}
                ...
                {disputeOrderId === order.id && (
                  <div className="mt-4 bg-red-50 border border-red-200 rounded-xl p-4 space-y-3">
                    <p className="text-sm font-medium text-red-700">申請押金賠償</p>
                    <textarea
                      value={disputeReason}
                      onChange={(e) => setDisputeReason(e.target.value)}
                      rows={3}
                      placeholder="請說明商品損壞情況..."
                      className="muji-input w-full resize-none text-sm"
                    />
                    <div className="flex gap-2">
                      <button type="button" onClick={() => { setDisputeOrderId(null); setDisputeReason(''); }}
                        className="px-3 py-1.5 border border-border rounded-lg text-sm text-muted hover:bg-surface transition-colors">
                        取消
                      </button>
                      <button onClick={() => handleDispute(order.id)} disabled={submittingAction || !disputeReason.trim()}
                        className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium disabled:opacity-60 transition-colors">
                        {submittingAction ? '提交中...' : '送出申請'}
                      </button>
                    </div>
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
                  {order.status === 'pending' && (
                    <span className="flex items-center gap-1.5 text-muted text-xs">
                      <Clock className="w-3.5 h-3.5" /> 等待買家付款
                    </span>
                  )}

                  {order.status === 'confirmed' && !order.seller_confirmed && (
                    <button
                  ...
                  {order.status === 'confirmed' && order.seller_confirmed === 1 && !order.buyer_confirmed && (
                    <span className="flex items-center gap-1.5 text-muted text-xs">
                      <Clock className="w-3.5 h-3.5" /> 已確認，等待買家確認
                    </span>
                  )}

                  {order.status === 'handover' && sellerPhotos.length === 0 && (
                  ...
                  {order.status === 'handover' && sellerPhotos.length > 0 && buyerPhotos.length === 0 && (
                    <span className="flex items-center gap-1.5 text-muted text-xs">
                      <CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> 已上傳，等待買家上傳
                    </span>
                  )}
                  {order.status === 'handover' && sellerPhotos.length > 0 && buyerPhotos.length > 0 && (
                  ...
                  {order.status === 'returning' && returnPhotos.length > 0 && (
                    <>
                      <button
                        onClick={() => handleComplete(order.id)}
                        disabled={submittingAction}
                        className="flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-medium px-4 py-2 rounded-full transition-colors disabled:opacity-60"
                      >
                        <CheckCircle2 className="w-3.5 h-3.5" /> 確認商品完好，完成租借
                      </button>
                      <button
                      ...
                      >
                        <ShieldAlert className="w-3.5 h-3.5" /> 申請押金賠償
                      </button>
                    </>
                  )}
                  {order.status === 'returning' && returnPhotos.length === 0 && (
                    <span className="flex items-center gap-1.5 text-muted text-xs">
                      <Clock className="w-3.5 h-3.5" /> 等待買家上傳歸還照片
                    </span>
                  )}

                  {order.status === 'completed' && !order.seller_reviewed && (
                    <button
                    ...
                    >
                      <Star className="w-3.5 h-3.5" /> 評價租借人
                    </button>
                  )}
                  {order.status === 'completed' && order.seller_reviewed === 1 && (
                    <span className="flex items-center gap-1.5 text-green-600 text-xs">
                  ...
                  {['confirmed', 'handover', 'active', 'returning'].includes(order.status) && (
                    <button
                      onClick={() => setChatOrder({ id: order.id, name: order.buyer_name })}
                ...
                {reviewingOrder === order.id && (
                  <form onSubmit={handleReview} className="mt-4 pt-4 border-t border-border space-y-3">
                    <p className="text-sm font-medium text-primary">為租借人留下評價</p>
                    <StarRating rating={rating} onChange={setRating} size="lg" />
                    <textarea
                      value={comment}
                      onChange={(e) => setComment(e.target.value)}
                      rows={3}
                      placeholder="分享這次出租的體驗..."
